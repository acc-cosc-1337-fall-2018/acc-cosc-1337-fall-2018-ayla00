...
{
	double hours_angle = ((clock.get_hours() / 12.0 * 360.0) + (clock.get_minutes()/2) + 180);
...
void ClockGraphic::draw_hours_text()
{
	for (int i = 0; i < 12; i++)
	{
		...
		std::unique_ptr<Shape> line = std::make_unique<Text>(device_context,
...
void ClockGraphic::draw_seconds_markers()
{
	for (int i = 0; i < 60; i++)
	{
		...
		if (i % 5 == 0)
		{
			device_context->SetPen(wxPen(wxColor(0, 0, 0), 3));   //overwrites device from above to make thicker lines
		}

		std::unique_ptr<Shape> line = std::make_unique<Line>(device_context,
